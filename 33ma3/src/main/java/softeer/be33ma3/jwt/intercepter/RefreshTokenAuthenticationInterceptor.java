...
public class RefreshTokenAuthenticationInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if(jwtProvider.validationToken(refreshToken)){  //유효한 리프레시 토큰인 경우
            ...
            return isSameMember(memberId, member);  //리프레시 토큰에서 꺼낸 멤버 아이디와, 리프레시 토큰으로 찾은 멤버가 같은지 확인
        }
    ...
    private boolean isSameMember(Long memberId, Member findMember){
        return memberId.equals(findMember.getMemberId());
    }
}
